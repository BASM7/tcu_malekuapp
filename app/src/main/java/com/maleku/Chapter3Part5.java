package com.maleku;

import android.os.Bundle;
import android.view.View;

public class Chapter3Part5 extends MalekuCompatActivity {
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.h3p5);
    }

    public void goPrevious(View view){
        switchActivity(this, Chapter3Part4.class);
    }
    public void goNext(View view){
        switchActivity(this, Chapter3Part6.class);
    }

    public void audio1 (View view) {
        play(R.raw.pag43_recuadrosuperior);
    }

    public void audio2_1 (View view) {
        play(R.raw.pag43_globo_primeraparte);
    }

    public void audio2_2 (View view) {
        play(R.raw.pag43_globo_segundaparte);
    }

    public void audio2_3 (View view) {
        play(R.raw.pag43_globo_terceraparte);
    }

    public void audio3 (View view) {
        play(R.raw.pag43_segundorecuadro);
    }
}